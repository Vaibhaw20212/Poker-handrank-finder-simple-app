...
# Drag-and-drop style entry or visual interface could go here
...
for i in range(len(st.session_state.players)):
    ...
    for j in range(5):
        key = f"p{i}_c{j}"
        card = cols[j].text_input(f"Card {j+1}", key=key)
        current_hand.append(card.strip().upper())

    parsed = [parse_card(c) for c in current_hand]
    card_ids = [c for c in current_hand]

    # Check if card changed from last check to re-enable button
    stored_hand_key = f"stored_hand_{i}"
    prev_hand = st.session_state.get(stored_hand_key, [""] * 5)
